    ...
    public function create($attributes = array(), $secret_key = '', $action = 'decrypt')
    {
        ...
        try{
            if ($action == 'encrypt') {
                ...
                $code = 200;            
                $content =  ['status'=>$code,  "httpStatus"=> $code,"codeDescription"=> "Success",  'apiResponse'=>$output];
                ...
                $content =  ['status'=>$code,"httpStatus"=> $code,"codeDescription"=> "Failed", "message"=> $msg];
                $result = ['content' => $content, 'code' => $code];
            }
        } catch(Exception $e){
            $output = $e->getMessage();
